...
⚙️ **後台設定頁面**
  ...
  　• 「複製時保留原始分行」開關（預設：關），開啟後選取文字複製時保留與判決頁面相同之斷行，關閉時則去除分行合併為連續段落
...
🆕 **v0.3.1 更新**
  • **「下載」耳標新增「一鍵複製全文」按鈕**：一鍵將當前判決正文複製到剪貼簿，保留判決頁面內之原始分行（不合併軟斷行），且一律不在尾端附上「（XX意旨參照）」，亦不寫入判決剪貼簿卡片
  • **後台新增「複製時保留原始分行」選項**（預設關）：開啟後，選取判決文字並以 Cmd/Ctrl+C 複製時保留與判決頁面相同之斷行，適合複製特定段落並維持原貌；關閉時沿用既有行為（去除分行、合併為連續段落）
...
🆕 **v0.2.6 更新**
  ...
  • **正文引文括號長駐高亮**：所有 `（…）` 附記以柔和天藍灰底長駐高亮，閱讀時可快速掃視；八類案號以類別分色（主題色／暖琥珀／中性灰／藕紫／深玫瑰紅／淺玫瑰紅／青綠深／青綠淡）
...
⚙️ **Options page**
  ...
  • Toggle **"preserve original line breaks on copy"** (default: off); when on, copying a selection keeps the on-page line breaks instead of merging them into a continuous paragraph
...
🆕 **v0.3.1 changes**
  • **New "Copy full text" button on the Download tab**: copies the current ruling's body straight to the clipboard while preserving the original on-page line breaks (no soft-wrap merging), never appending the `（…意旨參照）` citation suffix, and without adding a clipboard card
  • **New "preserve original line breaks" option** (default off): when on, copying a selection with Cmd/Ctrl+C keeps the on-page line breaks — for copying a specific passage verbatim; when off, line breaks are stripped and merged into a continuous paragraph as before
...
🆕 **v0.2.6 changes**
  ...
  • **Persistent inline highlight** on every `（…）` citation marker (soft sky-blue tint); case badges are color-coded by category — Supreme Court judgment (theme color) / Grand Chamber (warm amber) / chamber resolution (neutral grey) / symposium (lilac) / Grand Justices interpretation (deep rose) / Constitutional Court (light rose) / Supreme Administrative judgment (deep teal) / Supreme Administrative joint conference (light teal)
